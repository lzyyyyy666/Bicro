...
def main(opt):
    ...
    all_loss = [[], []]
    validate(opt, val_loader, [model_A, model_B])  # 这里出错了！！！

    # Warmup
    print("\n* Warmup")
    if opt.warmup_model_path:
        if os.path.isfile(opt.warmup_model_path):
            checkpoint = torch.load(opt.warmup_model_path, weights_only=False)
        ...
        for epoch in range(0, opt.warmup_epoch):
            ...
            # save_checkpoint(
            #     {
            #         "epoch": opt.warmup_epoch - 1,
            #         "model_A": model_A.state_dict(),
            #         "model_B": model_B.state_dict(),
            #         "opt": opt,
            #     },
            #     is_best=True,
            #     filename="checkpoint_{}.pth.tar".format(opt.warmup_epoch),
    ...
    for epoch in range(start_epoch, opt.num_epochs):
        ...
        pred_A = split_prob(prob_A, opt.p_threshold)  # pred 为bool数组，记录ITP是否为预测噪声对，即y=1/0， y=1为clean
...
def train(opt, net, net2, labeled_trainloader, unlabeled_trainloader=None, epoch=None):
    ...
    for i, batch_train_data in enumerate(labeled_trainloader):
        ...
        if epoch < (opt.num_epochs // 2):
            ...
            with torch.no_grad():
                net2.val_start()
                c_y, n_y = net2.predict(batch_images_l, batch_text_l, batch_lengths_l, img_lengths=img_lengths)
        else:
            with torch.no_grad():
                net2.val_start()
                c_y, n_y = net2.predict(batch_images_l, batch_text_l, batch_lengths_l, batch_images_u, batch_text_u,
                                        batch_lengths_u, epoch=epoch, img_lengths=img_lengths)
...
def warmup(opt, train_loader, model, epoch):
    ...
    for i, (images, captions, lengths, _, img_lengths) in enumerate(train_loader):
        data_time.update(time.time() - end)
        ...
        # Update the model
        loss = model.train(images, captions, lengths,
                           mode=opt.warmup_type, img_lengths=img_lengths)
...
def validate(opt, val_loader, models=[]):
    ...
    for ind in range(len(models)):
        ...
        )  # img_embs,cap_embs = [1000,1024]

        # clear duplicate 5*images and keep 1*images FIXME
        img_embs = np.array([img_embs[i] for i in range(0, len(img_embs), per_captions)])  # img_embs = [200,1024]

        # record computation time of validation
        start = time.time()
        print("Computing similarity from model {}".format(ind))
        # sims_mean += shard_attn_scores(  # sims_mean = [200,1000]
        #     models[ind], img_embs, cap_embs, cap_lens, opt, shard_size=100
        # )
        sims_mean += forward_sims(models[ind], img_embs, cap_embs)
        end = time.time()
        print(
    ...
    # caption retrieval
    (r1, r5, r10, medr, meanr) = i2t(img_embs.shape[0], sims_mean, per_captions)  # img_embs.shape[0]=200,per_captions=5
    print(
...
def eval_train(
    ...
    data_time = AverageMeter("data", ":6.3f")
    progress = ProgressMeter(
        len(data_loader), [batch_time, data_time], prefix="Computing losses"
    )
    ...
    for i, (images, captions, lengths, ids, img_lengths) in enumerate(data_loader):
        ...
        with torch.no_grad():
            # compute the loss
            loss_A = model_A.train(images, captions, lengths, mode="eval_loss", img_lengths=img_lengths)
            loss_B = model_B.train(images, captions, lengths, mode="eval_loss", img_lengths=img_lengths)
