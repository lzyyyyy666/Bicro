...
class RawImageDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenzier, opt, train, noise_ratio):
        self.opt = opt
        self.train = train
...
class PrecompRegionDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenizer, opt, train, noise_ratio):
...
def get_loader(data_path, data_name, data_split, tokenizer, opt, batch_size=100,
    ...
    if opt.precomp_enc_type == 'basic':
        dset = PrecompRegionDataset(data_path, data_name, data_split, tokenizer, opt, train, noise_ratio)
        ...
        dset = RawImageDataset(data_path, data_name, data_split, tokenizer, opt, train, noise_ratio)
        data_loader = torch.utils.data.DataLoader(dataset=dset,
                                                  batch_size=batch_size,
                                                  shuffle=shuffle,
                                                  num_workers=num_workers,
...
def get_loaders(data_path, data_name, tokenizer, batch_size, workers, opt):
    train_loader = get_loader(data_path, data_name, 'train', tokenizer, opt,
                              batch_size, True, workers, noise_ratio=opt.noise_ratio)
