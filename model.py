...
from lib_gpo.modules.aggr.gpo import GPO
...
class EncoderImage(nn.Module):
    ...
    def forward(self, images, img_lengths):
        ...
        img_emb, pool_weights = self.gpool(img_emb, img_lengths)
...
# class ContrastiveLoss_gpo(nn.Module):
#     """
#     Compute contrastive loss (max-margin based)
#     """
#
#     def __init__(self, opt, margin=0, max_violation=False):
#         super(ContrastiveLoss_gpo, self).__init__()
#         self.opt = opt
#         self.margin = margin
...
class SGRAF(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.bert = BertModel.from_pretrained('bert-base-uncased')
        self.linear = nn.Linear(768, opt.embed_size)
        self.gpool = GPO(32, 32)
    ...
    def forward_emb(self, images, captions, lengths, img_lengths):
        ...
        img_embs = self.img_enc(images, img_lengths)
        cap_embs = self.txt_enc(captions, lengths)
        return img_embs, cap_embs, lengths

    def forward_sim(self, img_embs, cap_embs):
    ...
    def train(
        ...
        img_embs, cap_embs, cap_lens = self.forward_emb(images, captions, lengths, img_lengths)
        sims = self.forward_sim(img_embs, cap_embs)
    ...
    def predict(self, images, captions, lengths, images_n='', captions_n='', lengths_n='', epoch=0, img_lengths=None):
        """
        predict the given samples
        """
        # compute the embeddings
        img_embs, cap_embs, cap_lens = self.forward_emb(images, captions, lengths, img_lengths)
        img_g, text_g = glo_emb(self, img_embs, cap_embs, cap_lens)
        sims = self.forward_sim(img_embs, cap_embs)
        ...
        if epoch:
            img_embs, cap_embs, cap_lens = self.forward_emb(images_n, captions_n, lengths_n, img_lengths)
            img_g_n, text_g_n = glo_emb(self, img_embs, cap_embs, cap_lens)
